...
# Diagnostic task IDs remain separate from the segmented tasks 3 and 4.
DUAL_FRANKA_TASKS.update(
    {
        103: FrankaTask(
            name="manual_primitive_test",
            instruction="Wait for explicit operator commands; execute one primitive at a time.",
            success_criteria="Report each requested primitive result to the operator.",
            constraints=("No autonomous action sequence or VLA.",),
        ),
        104: FrankaTask(
            name="vla_deployment_test",
            instruction="Wait for explicit operator commands to test VLA deployment.",
            success_criteria="Record inference and execution diagnostics; operator evaluates behavior.",
            constraints=("No planner or automatic action sequence.",),
        ),
    }
)
