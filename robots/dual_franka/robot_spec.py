...
def _add_cli_args(parser: argparse.ArgumentParser, use_dashboard: bool) -> None:
    ...
    parser.add_argument(
        "--instruction", default=None, help="Override the task instruction"
    )
...
def _parse_config(args: argparse.Namespace) -> RunConfig:
    ...
    return RunConfig(
        ...
        prompt_vars={
            ...
            "task_name": task.name,
            "instruction": getattr(args, "instruction", None) or task.instruction,
...
def _env_server_command(
    ...
    *,
    ...
    command = [
        ...
        str(port),
        "--task-description",
        getattr(args, "instruction", None) or task.instruction,
        "--calibration-path",
...
def _init_runtime(
    ...
    if args.task_id is not None:
        task = get_dual_franka_task(args.task_id)
        needs_vla = needs_vla or task.name.endswith("_vla") or args.task_id == 104
    ...
    connectors = {
        "env": lambda rpc: {
            ...
            "task_description": getattr(args, "instruction", None)
            or get_dual_franka_task(args.task_id).instruction,
        },
